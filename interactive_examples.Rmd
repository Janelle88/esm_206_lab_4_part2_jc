---
title: "Interactive Graph Examples"
author: "Janelle Christensen"
date: "10/22/2019"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

Here, we'll practice creating some interactive graphs and tables using built-in datasets in R packages.

- Interactive plots: {plotly}
- Interactive tables: {DT}

```{r, include = FALSE}

# Attach packages:

library(tidyverse)
library(plotly)
library(DT)

```

### A. Interactive graph of iris dimensioins

Using Edgar Anderson's famous "iris' dataset

```{r, echo = FALSE}

iris_graph <- ggplot(data = iris, aes(x = Petal.Width, y = Petal.Length)) +
  geom_point(aes(size = Sepal.Width, 
                 color = Species),
             alpha = 0.5) +
  theme_minimal() +
  labs(x = "Petal Width (cm)",
       y = "Petal Length (cm)",
       title = "MY IRIS GRAPH")

# iris_graph
# highlight all lines you want to comment out and press ctrl + shift + c 
# add eval = FALSE to have r not run a certain code chunk

ggplotly(iris_graph)

```

### Now I'll make an interactive table with DT

```{r, echo = FALSE}

datatable(msleep)

```

